...
from com760cw2_group11.msg import *
...
def change_state(state):
    ...
    if state_ == 0:
        ...
        leave_points_.append(position_)
    if state_ == 1:
...
def is_target_unreachable():
    ...
    for i in range(len(leave_points_)):
        for j in range(i+1, len(leave_points_)):
            if math.sqrt((leave_points_[i].x - leave_points_[j].x)**2 + 
                         (leave_points_[i].y - leave_points_[j].y)**2) < 0.1:
...
def main():
    ...
    pub_turn_right_signal = rospy.Publisher('/group1_homing_signal', group1homingsignal, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        if state_ == 0:
            ...
            if is_target_unreachable():
                ...
                turn_right_signal = group1homingsignal()
                turn_right_signal.instructionID = 1
                pub_turn_right_signal.publish(turn_right_signal)
